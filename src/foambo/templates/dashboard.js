...
function app(){return{
  ...
  state:{status:null,experiment:null,trials:null,objectives:null,streaming:null,generation:null,pareto:null,events:null,lineage:null},
  ...
  async init(){(async()=>{const l=await this.f('bootstrap-lineage');if(l&&!l._error&&Object.keys(l).length)this.state.lineage=l})();await this.r();this._schedPoll();this.$watch('tab',()=>this.r());document.addEventListener('keydown',e=>{if(e.target.tagName==='INPUT')return;const n=parseInt(e.key);if(n>=1&&n<=this.tabs.length)this.tab=this.tabs[n-1].id;if(e.key==='v'&&!e.ctrlKey&&!e.metaKey){const hovered=document.querySelector('.chart-card:hover');if(hovered){const plotDiv=hovered.querySelector('.js-plotly-plot')||(hovered.classList.contains('js-plotly-plot')?hovered:null);if(plotDiv&&plotDiv.data){this.enlargedEl=true;this.$nextTick(()=>{const target=document.getElementById('enlarge-target');if(target)Plotly.react(target,plotDiv.data,{...plotDiv.layout,width:null,height:null},PC)})}}}if(e.key==='Escape'){this.enlargedEl=null;const et=document.getElementById('enlarge-target');if(et)Plotly.purge(et)}});this.$watch('xt',async(v)=>{this.td=null;this.tdViz=null;this.tdVizLoading=false;this.tdVizErr='';if(v!==null){const r=await this.f('trials/'+v);if(r&&!r._error)this.td=r}})},
  _etags:{},
  ...
  fmtTime(s){if(s==null)return'--';if(s<60)return s.toFixed(1)+'s';if(s<3600)return Math.floor(s/60)+'m '+Math.round(s%60)+'s';const h=Math.floor(s/3600);return h+'h '+Math.floor((s%3600)/60)+'m'},
  getDepNames(){
    const names=new Set();
