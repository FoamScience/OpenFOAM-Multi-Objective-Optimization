...
:
    ...
    fieldComponents_(dict.get<wordList>("fieldComponents")),
...
bool Foam::functionObjects::smartSimFunctionObject::end()
{
    ...
    Info << "Field components: " << fieldComponents_ << endl;
    Info << "Field dimensions: " << fieldDimensions_ << endl;

    word prefix = mesh_.time().globalCaseName() + "_";
    ...
    forAll(fieldNames_, fieldI)
    {
        ...
        if(fieldDimensions_[fieldI] == 1) // scalar field
        {
            ...
            client_.put_tensor(prefix + sField.name(), (void*)sField.internalField().cdata(), dims,
        ...
        else if (fieldDimensions_[fieldI] == 3) // vector field
        {
            ...
            // Send the cell-centered scalar field to SmartRedis
            client_.put_tensor(prefix + vField.name(), (void*)vField.internalField().cdata(), dims,
                               SRTensorTypeDouble, SRMemLayoutContiguous);
        }
    ...
    // Set field dimensions 
    //std::vector<size_t> dims = {size_t(mesh_.nCells()), 1, 1};

    for(auto component : fieldComponents_)
    {
        label pos = component.find("_");
        word fieldName = component.substr(0, pos);
        component.erase(0, pos + 1);
        Info<< fieldName << component << endl;
        // Get the cell-centered scalar field from the mesh (registry).
        const volVectorField& sField = mesh_.lookupObject<volVectorField>(fieldName);
        HashTable<label> map;
        map.insert("x", 0);
        map.insert("y", 1);
        map.insert("z", 2);

        scalarField cmpVal = sField.internalField().component(map[component]);
        scalarField cmpPos = mesh_.C().component(map[component]);
        for (const auto& patch: mesh_.boundaryMesh()) {
            if (patch.name() == "lowerWall") {
                auto cells = patch.faceCells();
                scalarField cmp(cells.size());
                scalarField val(cells.size());
                forAll(cmp, ci) {
                    cmp[ci] = cmpPos[cells[ci]];
                    val[ci] = cmpVal[cells[ci]];
                }
                std::vector<size_t> dims = {size_t(cells.size()), 1, 1};
                // Send the cell-centered scalar field to SmartRedis
                client_.put_tensor(prefix + fieldName + "_" + component, (void*)val.cdata(), dims,
                                   SRTensorTypeDouble, SRMemLayoutContiguous);
                client_.put_tensor(prefix + component, (void*)cmp.cdata(), dims,
                                   SRTensorTypeDouble, SRMemLayoutContiguous);
            }
        }
    }
